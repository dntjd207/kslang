...
class UpdateBlogPostRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $blogPost = $this->route('blog_post') ?? $this->route('blogPost');

        return [
            'save_action' => ['required', 'string', 'in:draft,publish,archive'],
            'slug' => [
                'nullable',
                'string',
                'max:255',
    ...
    {
        return [
            'save_action.required' => '저장 방식을 선택해주세요.',
            'save_action.in' => '올바른 저장 방식이 아닙니다.',
